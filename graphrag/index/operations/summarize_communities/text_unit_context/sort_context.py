    ...
    if sub_community_reports:
        ...
        if not report_df.empty:
            if report_df[schemas.COMMUNITY_ID].dtype == float:
                report_df[schemas.COMMUNITY_ID] = report_df[schemas.COMMUNITY_ID].astype(int)
            report_string = f"----REPORTS-----\n{report_df.to_csv(index=False, sep=',')}"
            contexts.append(report_string)

    text_units = [unit for unit in text_units if "id" in unit and unit["id"] and str(unit["id"]).strip() != ""]
...
def sort_context(
    ...
    sorted_text_units = sorted(local_context, key=lambda x: x[schemas.ENTITY_DEGREE], reverse=True)
    ...
    for record in sorted_text_units:
        ...
        if max_context_tokens:
            new_context_string = get_context_string(current_text_units, sub_community_reports)
