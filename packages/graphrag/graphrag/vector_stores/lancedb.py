...
class LanceDBVectorStore(BaseVectorStore):
    ...
    def load_documents(self, documents: list[VectorStoreDocument], overwrite: bool = True) -> None:
        ...
        for document in documents:
            self.vector_size = len(document.vector) if document.vector else self.vector_size
        ...
        if overwrite:
            ...
            self.document_collection = self.db_connection.open_table(self.index_name if self.index_name else "")
            if data:
    ...
    def filter_by_id(self, include_ids: list[str] | list[int]) -> Any:
        ...
        if len(include_ids) == 0:
            ...
            if isinstance(include_ids[0], str):
                ...
                self.query_filter = f"{self.id_field} in ({', '.join([str(id) for id in include_ids])})"
    ...
    def similarity_search_by_vector(
        ...
        if self.query_filter:
            docs = (
                self.document_collection.search(query=query_embedding, vector_column_name=self.vector_field)
    ...
    def search_by_id(self, id: str) -> VectorStoreDocument:
        ...
        doc = self.document_collection.search().where(f"{self.id_field} == '{id}'", prefilter=True).to_list()
        if doc:
